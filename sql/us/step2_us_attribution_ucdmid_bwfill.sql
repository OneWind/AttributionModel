...
from (select * from a.feng_us_factvisits_ucdmid_bwfill
      where ucdmid in (select ucdmid from a.feng_us_ucdmidpool))
go
...
select ucdmid, servertimemst, ustimezone, min(signupcreatedate) signupcreatedate, 
...
group by ucdmid, servertimemst, ustimezone, subchannel, 
