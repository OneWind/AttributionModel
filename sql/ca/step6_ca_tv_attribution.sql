select dropif('a', 'feng_ca_visit_tv_signup_tmp')
go

select tb1.*
     , tvcount
     , imps
     , case when catimezone in ('EST', 'CST', 'AST') then 'east'
            when catimezone in ('PST', 'MST') then 'west'
            else 'non-ca' end as coast
into a.feng_ca_visit_tv_signup_tmp
...
--select count(*) from a.feng_ca_visit_tv_signup_tmp
--go
--2,753,769

select dropif('a', 'feng_ca_visit_tv_signup_tmp2')
go
...
into a.feng_ca_visit_tv_signup
from a.feng_ca_visit_tv_signup_tmp2 tb1
    left join (select *, 'west' coast
               from a.feng_catv_weights_west) tb2
        on (substr(cast(tb1.servertimemst as varchar(100)), 1, 16) = substr(cast(tb2.datetime as varchar(100)), 1, 16)
            and tb1.coast = tb2.coast)
go

drop table a.feng_ca_visit_tv_signup_tmp
go
drop table a.feng_ca_visit_tv_signup_tmp2
go


alter table a.feng_ca_visit_tv_signup add tvweights_ft float
go
alter table a.feng_ca_visit_tv_signup add tvweights_lt float
go
alter table a.feng_ca_visit_tv_signup add tvweights_mt float
go
alter table a.feng_ca_visit_tv_signup add firsttouchweights_update float
go
alter table a.feng_ca_visit_tv_signup add lasttouchweights_update float
go
alter table a.feng_ca_visit_tv_signup add multitouchweights_update float
go


update a.feng_ca_visit_tv_signup
...
from a.feng_ca_visit_tv_signup
go
update a.feng_ca_visit_tv_signup
...
alter table a.feng_ca_visit_tv_signup add tvweights_ft_invisit float
go
alter table a.feng_ca_visit_tv_signup add tvweights_lt_invisit float
go
alter table a.feng_ca_visit_tv_signup add tvweights_mt_invisit float
go
...
select dropif('a', 'feng_ca_attribution_phase1')
go

select tb2.visitreferrer, tb2.pageurl, tb1.*
into a.feng_ca_attribution_phase1
from a.feng_ca_visit_tv_signup tb1
    join a.feng_ca_factvisits_ucdmid_bwfill tb2 on tb1.rdnum = tb2.rdnum
go
