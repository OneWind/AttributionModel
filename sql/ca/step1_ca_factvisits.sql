...
into a.feng_ca_factvisits_tmp
from (select tmp1.*
           , case when tmp1.visitcountryid = 95 then tmp2.state else 'non-ca' end as castate
           , case when tmp1.visitcountryid = 95 then tmp2.catimezone else 'non-ca' end catimezone
      from p.fact_visits tmp1
          left join (select visitregionid visitregionid, state, catimezone 
                     from a.feng_ca_timezones) tmp2
              on tmp1.visitregionid = tmp2.visitregionid
      where tmp1.siteid = 3717
...
--select subchannel, count(*) cnt from a.feng_ca_factvisits_tmp
--group by subchannel
--go
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- data saved in: a.feng_ca_factvisits_ucdmid_bwfill                          --
--------------------------------------------------------------------------------
select dropif('a', 'feng_ca_factvisits_tmp2')
go
...
into a.feng_ca_factvisits_ucdmid_bwfill
from 
    (
...
--                     , tb1.dnaorders
                ...
                from a.feng_ca_factvisits_tmp2 tb1
                    left join (select *
                               from a.feng_ca_factvisits_tmp2
...
--                         tb1.dnaorders, 
                         tb1.subchannel, tb1.castate, tb1.catimezone,
...
--                   dnaorders,
                   ...
                   visitreferrer, pageurl, castate, catimezone
         )
    ) 
go

select subchannel, count(*) cnt 
from a.feng_ca_factvisits_ucdmid_bwfill
group by subchannel
go
