...
select ucdmid, min(servertimemst) dnaorderdate
...
select * from (
select tb1.ucdmid, tb1.dnaorderdate, min(tb2.signupcreatedate) firstsudate
...
--------------------------------------------------------------------------------
select dropif('a', 'feng_us_dna_factvisitsample')
go
...
into a.feng_us_dna_factvisitsample
from (select * from a.feng_us_factvisits_ucdmid_bwfill
      where ucdmid in (select ucdmid from a.feng_us_dna_ucdmidpool))  tb1
go

select count(*) from a.feng_us_dna_factvisitsample
go

select top 10 * from a.feng_us_dna_factvisitsample
go
...
select tb1.ucdmid
     , tb1.servertimemst
     , tb2.servertimemst dnaorderdate
     , tb1.visitorid
     , tb1.subchannel
     , tb1.dnaorders
...
from a.feng_us_dna_factvisitsample tb1
    join (select ucdmid, servertimemst
          from a.feng_us_dna_factvisitsample
          where dnaorders > 0
         )tb2
        on (tb1.ucdmid = tb2.ucdmid 
            and tb1.servertimemst > dateadd(day, -30, tb2.servertimemst) 
